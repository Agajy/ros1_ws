...
class _AckermannCtrlr(object):
    ...
    def __init__(self):
        ...
        self._steering_cmd_sub = \
            rospy.Subscriber("steering", Float32,self.steering_cmd, queue_size=1)
        
        self._throttle_cmd_sub = \
            rospy.Subscriber("throttle", Float32,self.throttle_cmd, queue_size=1)
        
        self._target_pub = rospy.Publisher("vrpn_client_node/target/pose", PoseStamped, queue_size=10)

        self._gazebo_sub = rospy.Subscriber('/gazebo/model_states', ModelStates, self.gazebo_callback)
    ...
    def gazebo_callback(self, data):
        object_name = "ackermann_vehicle"  # Remplacez par le nom réel de l'objet
        if object_name in data.name:
            index = data.name.index(object_name)
            pose = data.pose[index]
            pose_stamped = PoseStamped()
            pose_stamped.header.stamp = rospy.Time.now()
            pose_stamped.header.frame_id = "map"

            # Appliquer la transformation inverse sur la position
            pose_stamped.pose.position.x = pose.position.x
            pose_stamped.pose.position.y = pose.position.z
            pose_stamped.pose.position.z = -pose.position.y

            # Appliquer la transformation inverse sur l'orientation
            quaternion = (
                pose.orientation.x,
                pose.orientation.y,
                pose.orientation.z,
            ...
            # Rotation inverse (-90° autour de X)
            inverse_rotation_quaternion = tf.transformations.quaternion_from_euler(-1.5708, 0, 0)  # -90° en radians
            new_quaternion = tf.transformations.quaternion_multiply(inverse_rotation_quaternion, quaternion)

            pose_stamped.pose.orientation.x = new_quaternion[0]
            pose_stamped.pose.orientation.y = new_quaternion[1]
            pose_stamped.pose.orientation.z = new_quaternion[2]
    ...
    def throttle_cmd(self, msg):
        self._last_cmd_time = rospy.get_time()
        with self._ackermann_cmd_lock:
            self._speed = -msg.data
        
    
    def steering_cmd(self, msg):
        self._last_cmd_time = rospy.get_time()
        with self._ackermann_cmd_lock:
